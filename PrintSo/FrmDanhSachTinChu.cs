	...
	public partial class FrmDanhSachTinChu : Form
	{
		...
		private void FrmDanhSachTinChu_Load(object sender, EventArgs e)
		{
			try
			{
				...
				this.splitContainer1.SplitterDistance = this.btnPrintPreview.Width + 20;
				...
				this.cbxFontName.Width = this.splitContainer1.Panel1.Width - 15;
		...
		private void FillTinChu()
		{
			try
			{
				...
				list.Add(new ExcelPerson
				{
					...
					GenderCell = new PrintHelper.Cell(),
					GenderCell = 
					{
						Value = "Giới tính",
						HAlignment = 2,
					...
					AddressCell = new PrintHelper.Cell(),
					AddressCell = 
					{
						Value = "Địa chỉ",
						HAlignment = 2,
		...
		private void PageSetting(Worksheet curWS, int row)
		{
			...
			curWS.SetRowsHeight(0, curWS.RowCount, 30);
			curWS.SetColumnsWidth(0, 1, 60);
			curWS.SetColumnsWidth(1, 1, 280);
			curWS.SetColumnsWidth(2, 1, 65);
			curWS.SetColumnsWidth(3, 1, 65);
			curWS.SetColumnsWidth(4, 1, 100);
			curWS.SetColumnsWidth(5, 1, 110);
			curWS.SetColumnsWidth(6, 1, 500);
			curWS.SetColumnsWidth(7, 1, 300);
			this.updateCol();
			...
			try
			{
				for (int i = 0; i < this.curWS.ColumnCount; i++)
				{
					if (!this.curWS.ColumnHeaders[i].IsVisible && !this.contextMenuStripCol.Items.ContainsKey(i.ToString()))
					{
						...
						toolStripMenuItem.Text = "Hiện cột '" + this.curWS.ColumnHeaders[i].RenderText + "'";
